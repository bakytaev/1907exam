...
def employee_detail(employee_id):
    ...
    if employee:
        ...
        flash('Такого сотрудника не существует', category='danger')
        return redirect(url_for('index'))
    ...
    if employee:
        ...
        if employee.user_id == current_user.id:
            if request.method == 'POST':
                ...
                db.session.commit()
                flash('Вы успешно данные о сотруднике!', category='success')
                return redirect(url_for('index'))
            else:
                return render_template('employee_delete.html', form=form, employee=employee)
        else:
            ...
            if request.method == 'POST':
                ...
                if form.errors:
                    for errors in form.errors.values():
                        for error in errors:
                            flash(error, category='danger')
                return render_template('post_edit.html', employee=employee, form=form)
            else:
